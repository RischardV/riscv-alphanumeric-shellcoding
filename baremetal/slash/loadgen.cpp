...
static string build_instr_txt(const txt_t &txt, reg_t reg)
{
    stringstream store;
    ...
    switch(reg) {
        case 20: /* s4 */
            store << "amoor.w    t5,s4,(sp)";
            break;
        case 22: /* s6 */
            store << "amoor.w    t5,s6,(sp)";
            break;
        case 6: /* t1 */
            store << "amoor.w.aq t5,t1,(sp)";
            break;
        default:
            store << "undefstore (" << reg << ")";
            break;
    }
    store << endl;
...
static void try_add_value(sequences_t &t, const instr_t &e)
{
    ...
    string txt = build_instr_txt(e.txt, e.dst);
    seq_t seq = {.valid = 1, .cost = e.cost, .value = e.value, .txt = txt};
    t[lo] = seq;
}
